...
public class NotificationService {
        ...
        @Transactional
        public void notifySessionBookingRequest(SessionBookingEntity session) {
                UserEntity provider = session.getProvider();
                String clientName = session.getClient().getName();

                String message = "New session request from " + clientName;

                createNotification(provider, NotificationType.SESSION_BOOKING_REQUEST, message, session.getId());
        }
        ...
        public void notifyBookingReminder(BookingEntity booking) {
                ...
                createNotification(practitioner, NotificationType.SESSION_REMINDER, practitionerMessage,
                                booking.getId());
        }
        ...
        public void notifyBookingCancelled(BookingEntity booking, UserEntity canceller) {
                ...
                String message = String.format("🚫 %s has cancelled the session scheduled for %s", cancellerName,
                                dateTime);
        ...
        private void createNotification(UserEntity recipient, NotificationType type, String message, Long relatedId) {
                ...
                if (notificationRepository.existsByRecipient_IdAndTypeAndRelatedBookingId(recipient.getId(), type,
                                relatedId)) {
                        return;
                }
