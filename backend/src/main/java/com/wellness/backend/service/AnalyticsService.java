...
public class AnalyticsService {
    ...
    public PractitionerAnalyticsDTO getPractitionerAnalytics(Long practitionerId) {
        ...
        BigDecimal sessionToday = orZero(
                bookingRepository.sumSessionRevenueByPractitionerAndDateRange(practitionerId, todayStart, now));
        BigDecimal productToday = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, todayStart, now));
        BigDecimal totalToday = sessionToday.add(productToday);

        BigDecimal sessionYesterday = orZero(bookingRepository
                .sumSessionRevenueByPractitionerAndDateRange(practitionerId, yesterdayStart, todayStart));
        BigDecimal productYesterday = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, yesterdayStart, todayStart));
        BigDecimal totalYesterday = sessionYesterday.add(productYesterday);

        // Weekly
        BigDecimal sessionThisWeek = orZero(
                bookingRepository.sumSessionRevenueByPractitionerAndDateRange(practitionerId, weekStart, now));
        BigDecimal productThisWeek = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, weekStart, now));
        ...
        BigDecimal productPrevWeek = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, prevWeekStart, weekStart));
        BigDecimal totalPrevWeek = sessionPrevWeek.add(productPrevWeek);

        // Monthly
        BigDecimal sessionThisMonth = orZero(
                bookingRepository.sumSessionRevenueByPractitionerAndDateRange(practitionerId, monthStart, now));
        BigDecimal productThisMonth = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, monthStart, now));
        BigDecimal totalThisMonth = sessionThisMonth.add(productThisMonth);

        BigDecimal sessionPrevMonth = orZero(bookingRepository
                .sumSessionRevenueByPractitionerAndDateRange(practitionerId, prevMonthStart, monthStart));
        BigDecimal productPrevMonth = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, prevMonthStart, monthStart));
        BigDecimal totalPrevMonth = sessionPrevMonth.add(productPrevMonth);

        // Yearly
        BigDecimal sessionThisYear = orZero(
                bookingRepository.sumSessionRevenueByPractitionerAndDateRange(practitionerId, yearStart, now));
        BigDecimal productThisYear = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, yearStart, now));
        ...
        BigDecimal productPrevYear = orZero(
                orderRepository.sumProductRevenueByProviderAndDateRange(practitionerId, prevYearStart, yearStart));
    ...
    public PatientAnalyticsDTO getPatientAnalytics(Long userId) {
        ...
        List<BookingResponseDTO> recentSessions = bookingRepository.findTop5ByUser_IdOrderByBookingDateDesc(userId)
    ...
    private BookingResponseDTO mapToBookingDTO(com.wellness.backend.model.BookingEntity b) {
        ...
        dto.setPractitionerId(b.getPractitioner().getId());
